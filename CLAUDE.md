...
- `packages/mobile` — Flutter app (not part of the npm workspaces; use `flutter test` / `flutter analyze` inside it). `lib/core/` is a pure-Dart port of the crypto core kept byte-compatible with `@vaultkeep/crypto` — the compat fixtures in `test/fixtures/` are generated by the node core, so any change to the shared crypto formats must regenerate them.
