...
- `packages/mobile` — Flutter app (not part of the npm workspaces; use `flutter test` / `flutter analyze` inside it). `lib/core/` is a pure-Dart port of the crypto core kept byte-compatible with `@vaultkeep/crypto` — the compat fixtures in `test/fixtures/` are generated by the node core, so any change to the shared crypto formats must regenerate them. `lib/core/biometric.dart` mirrors desktop's `biometric.ts` contract (wrap a copy of the master key behind the OS keystore, gated by a biometric prompt) with injected `SecureEnclave`/`TokenStore` interfaces for testability; the real `local_auth` + `flutter_secure_storage` binding lives in `lib/platform/biometric_platform.dart`, kept separate so the core stays platform-channel-free in tests.
